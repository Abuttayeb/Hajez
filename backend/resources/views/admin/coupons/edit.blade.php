...
@section('content')
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="text-2xl font-extrabold">تعديل: {{ $coupon->code }}</h1>
    </div>

    <form method="POST" action="{{ route('admin.coupons.update', $coupon) }}" class="bg-white rounded-2xl shadow-sm p-6 max-w-2xl">
        @csrf @method('PUT')
